...
class role_drupal::update (
  ...
  if ( $role_drupal::updateall == true ) {
    exec { 'update_all':
      command   => 'docker-compose exec -T drupal drush pm-updatestatus --full | grep "Update available" | cut -d" " -f2 | xargs drush up --no-backup -u 1 -y',
    ...
    exec { 'update_security':
      command   => 'docker-compose exec -T drupal drush pm-updatestatus --full | grep "SECURITY" | cut -d" " -f2 | xargs docker-compose exec -T drupal drush up --no-backup -u 1 -y',
      path      => '/sbin:/usr/bin:/usr/local/bin/:/bin/',
      provider  => shell,
      cwd       => $role_drupal::repo_dir,
      user      => 'root',
